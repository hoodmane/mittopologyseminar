How to organize the Monday seminar:
...
- Haynes will send you a list of speakers and dates. Add the talk info to talks.json.
  talks.json file is structured like:
  ...
  Read talks.json for a description of the fields for each talk.

- Run make-index3.1.py (with no arguments) to regenerate the main page and the past seminars page using the talk data
  in talks.json, generate the posters that are linked from the webpage, and the emails.
...
- If talks have a weird schedule or you make changes to things less than a week ahead of the seminar, you should email
  Jon Harmon (harmonjo@math.mit.edu) and let him know so he can fix the calender / posters. You can check the entry on
  the math department calender here to see if Jon knows about any changes:
    http://events.mit.edu/scripts/monthly_ext.pl?groupid=2861&location=http://math.mit.edu/news/&type=php
...
- In the emails, all dollar signs and backslashes will be deleted, otherwise LaTeX will be left as-is.
  ...
  \mathcal{C}, you should include as a macro "\def\C{\mathcal{C}}" so that it will appear as just "C" rather than "mathcal{C}" in the email.
...
all talks must have a date, and they must all be distinct
requirements: python 2.5, (in particular, it definitely won't work on Python 3, Python 2.7 is fine)
can only handle talks that end before 7pm; all talks are 1 hours long
...
2:
I added the line "os.system('chmod 755 posters/%s.pdf' % (talk.name))"
...
I also added email.template etc.

I also messed with the poster template; I think it looks better now, but it does
...
TODO: Send no talk today email with "cancellation_reason" or generic one if no reason given but no talk given in between actual talks. Send an extra email warning about late week talks? Auto-detect no_email cases? Send email to organizer telling them to write their own email(s) in case of three talks or talks on different days on the same week?
